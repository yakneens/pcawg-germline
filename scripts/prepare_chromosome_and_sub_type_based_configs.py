...
def generate_config_objects(chromosome_names, sub_types, config_location):
...
def create_configs_command(args):
    ...
    sub_types = ["c_to_a", "c_to_t", "c_to_g", "t_to_g", "t_to_c", "t_to_a"]
    
    if (not os.path.isdir(config_location)):
        os.makedirs(config_location)
        
    for config in generate_config_objects(chromosome_names, sub_types, config_location):
