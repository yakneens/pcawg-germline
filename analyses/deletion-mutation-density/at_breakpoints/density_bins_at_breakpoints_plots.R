...
ggplot(breakpoint_densities_100_30_200_carriers_for_plot, aes(x=(bin_index - midpoint) * bin_width, y=snv_density,fill=abs(midpoint - bin_index) * bin_width)) + geom_bar(stat="identity") + facet_grid(.~breakpoint_type) + guides(fill=guide_legend(title="Distance from breakpoint")) + xlab("Distance from breakpoint") + ylab("SNV Density") + annotate("text", x=0, y=1.1*max_density, label="Breakpoint")
...
midpoint = ceiling(num_bins / 2)
ggplot(corrected_breakpoint_densities_100_30_200_carriers_for_plot, aes(x=(bin_index - midpoint) * bin_width, y=snv_density,fill=abs(midpoint - bin_index) * bin_width)) + geom_bar(stat="identity") + facet_grid(.~breakpoint_type) + guides(fill=guide_legend(title="Distance from breakpoint")) + xlab("Distance from breakpoint") + ylab("SNV Density") + annotate("text", x=0, y=1.1*max_density, label="Breakpoint")
...
breakpoint_densities_all_by_project_for_plot = rbind(do.call(rbind, lapply(names(summed_breakpoint_densities_all_by_project_start), function(x){data.table(rowSums(t(summed_breakpoint_densities_all_by_project_start[[x]])),seq(num_bins),x, "start")})),
                                                         do.call(rbind, lapply(names(summed_breakpoint_densities_all_by_project_end), function(x){data.table(rowSums(t(summed_breakpoint_densities_all_by_project_end[[x]])),seq(num_bins),x, "end")})))
